...
@js.JS('JSON.parse')
external Object jsonParse(String json);
...
void mergeFromJsonString(
  ...
  final parsed = jsonParse(json);
  _mergeFromRawJsMap(fs, parsed, registry);
}

void _mergeFromRawJsMap(FieldSet fs, Object json, ExtensionRegistry? registry) {
  fs.ensureWritable();
  ...
  for (final key in keys) {
    ...
    if (fi.isMapField) {
      _appendRawJsMap(
          meta, fs, js_util.getProperty(json, key), fi as MapFieldInfo<dynamic, dynamic>, registry);
    } else if (fi.isRepeated) {
      _appendRawJsList(meta, fs, js_util.getProperty(json, key), fi, registry);
    } else {
      _setRawJsField(meta, fs, js_util.getProperty(json, key), fi, registry);
    }
  }
}

void _appendRawJsList(BuilderInfo meta, FieldSet fs, Object jsonList,
  ...
  final length = js_util.getProperty(jsonList, 'length') as int;
  for (var i = 0; i < length; i++) {
    final value = js_util.getProperty(jsonList, i);
    var convertedValue =
        _convertRawJsValue(meta, fs, value, fi.tagNumber, fi.type, registry);
...
void _appendRawJsMap(BuilderInfo meta, FieldSet fs, Object jsonList,
  ...
  final length = js_util.getProperty(jsonList, 'length');
  ...
  for (var i = 0; i < length; i++) {
    ...
    final convertedKey = _convertRawJsValue(
        entryMeta,
        entryFieldSet,
        js_util.getProperty(value, mapKeyFieldNumber),
        mapKeyFieldNumber,
        fi.keyFieldType,
        registry);
    var convertedValue = _convertRawJsValue(
        entryMeta,
        entryFieldSet,
        js_util.getProperty(value, mapValueFieldNumber),
        mapValueFieldNumber,
        fi.valueFieldType,
        registry);
    // In the case of an unknown enum value, the converted value may return
...
void _setRawJsField(BuilderInfo meta, FieldSet fs, Object json, FieldInfo fi,
    ExtensionRegistry? registry) {
  final value =
      _convertRawJsValue(meta, fs, json, fi.tagNumber, fi.type, registry);
...
dynamic _convertRawJsValue(BuilderInfo meta, FieldSet fs, Object value, int tagNumber,
  ...
  switch (PbFieldTypeInternal.baseType(fieldType)) {
    ...
    case PbFieldTypeInternal.GROUP_BIT:
    case PbFieldTypeInternal.MESSAGE_BIT:
      if (js_util.isJavaScriptSimpleObject(value)) {
        final subMessage = meta.makeEmptyMessage(tagNumber, registry);
        _mergeFromRawJsMap(subMessage.fieldSet, value, registry);
        return subMessage;
      }
