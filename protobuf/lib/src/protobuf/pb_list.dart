...
class PbList<E> extends ListBase<E> {
  ...
  @override
  int get hashCode =>
      _HashUtils._hashObjects(_wrappedList);
...
/// A [ListBase] implementation used for protobuf `repeated` message fields.
class PbListMessage<E extends GeneratedMessage> extends PbList<E> {
  final List<E> _wrappedList;
  final CheckFunc<E> _check;
  ...
  PbListMessage({CheckFunc<E> check = _checkNotNull})
      : _wrappedList = <E>[],
        _check = check;

  PbListMessage.unmodifiable()
      : _wrappedList = const [],
        _check = _checkNotNull,
        _isReadOnly = true;

  PbListMessage.from(List from)
      : _wrappedList = List<E>.from(from),
        _check = _checkNotNull;
  ...
  @override
  bool operator ==(Object other) =>
      other is PbListMessage && _areListsEqual(other, this);

  @override
  int get hashCode {
    final wrapped = _wrappedList;
    var hash = 0;
    for (var i= 0; i < wrapped.length; i++) {
      hash = _HashUtils._combine(hash, wrapped[i].hashCode);
    }
    return hash;
  }
